...
//		s5k4ca_sensor_write_list(client,s5k4ca_preview_0,"s5k4ca_preview_0"); // 1024 x 768 
	...
	#if 1//PGH
		//AE/AWB UNLOCK 
		//printk("[PGH] AF_AWB_UNLOCK ON PREVIEW\n");
...
//		s5k4ca_sensor_write_list(client,s5k4ca_capture_0,"s5k4ca_capture_0"); // 2048x1536(3M)
		...
		{	
			...
			if (previous_scene_mode==3)//scene night
			{	
		...
		else
		{
			...
			delay = 50; // not sure but from now on no problem[PGH]
...
static int s5k4ca_sensor_af_control(struct i2c_client *client, int type)
{
    ...
    switch (type)
    {
        ...
        case 0: //[PGH] CASE 0 for AF Release

		//AE/AWB UNLOCK 
		//printk("[PGH]AF_AWB_UNLOCK on AF RELEASE~!!!\n");
		#if 1//PGH FOR SUNSET ERROR
...
//		s5k4ca_sensor_af_power_off(client);
		...
		s5k4ca_sensor_write(client, 0x002A, 0x030E);    
		s5k4ca_sensor_write(client, 0x0F12, 0x00F3);    
		msleep(250);
...
#if 1//PGH
		//AE/AWB LOCK 
		//printk("[PGH] AF_AWB_LOCK on AF START\n");
		...
		do_gettimeofday(&a);
            ...
            // delay 2frame : 260ms s5k4ca_sensor_write(client, 0x0F12, 0x0002) CND Write 후 해당 Delay time 이후에 AF Status Check
            msleep(260);
...
//bestiq            msleep(200);
	...
	case 2://AUTO
	     printk("[CAM-SENSOR] = CAM : 3M AF_AUTO SET \n");	
            s5k4ca_sensor_write(client, 0xFCFC, 0xD000);    
            s5k4ca_sensor_write(client, 0x0028, 0x7000);    
	...
	case 3://INFINITY
	     printk("[CAM-SENSOR] = CAM : 3M AF_INFINITY SET \n");	
            s5k4ca_sensor_write(client, 0xFCFC, 0xD000);    
            s5k4ca_sensor_write(client, 0x0028, 0x7000);    
	...
	case 4://MACRO
	     printk("[CAM-SENSOR] = CAM : 3M AF_MACRO SET \n");	
            s5k4ca_sensor_write(client, 0xFCFC, 0xD000);    
            s5k4ca_sensor_write(client, 0x0028, 0x7000);    
            s5k4ca_sensor_write(client, 0x002A, 0x161C);    
            s5k4ca_sensor_write(client, 0x0F12, 0xA2A8);    //Normal Mode Select
            ...
            s5k4ca_sensor_write(client, 0x002A, 0x030E);    
            s5k4ca_sensor_write(client, 0x0F12, 0x0050);   
			
	     msleep(200);		
		break;

	case 5://FIXED
	     printk("[CAM-SENSOR] = CAM : 3M AF_FIXED SET \n");	
            s5k4ca_sensor_write(client, 0xFCFC, 0xD000);    
            s5k4ca_sensor_write(client, 0x0028, 0x7000);    
...
static int s5k4ca_sensor_af_power_off(struct i2c_client *client)
{
	(printk("[CAM-SENSOR] +%s\n",__func__));
	int ret = 0;

	//	if(s5k4ca_sensor_write_list(client,s5k4ca_af_power_off,"s5k4ca_af_power_off") < 0)
	//		return -1;
...
static int s5k4ca_sensor_command(struct i2c_client *client, unsigned int cmd, void *arg)
{
	...
	switch (cmd)
	{
		...
		case USER_EXIT:
			ret = s5k4ca_sensor_af_power_off(client);			
			s5k4ca_sensor_exit();
			break;
